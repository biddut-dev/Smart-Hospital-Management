-- ============================================================================
-- Smart Hospital Management System - Database Schema & Sample Data
-- DBMS Lab Project
-- Technology: MySQL / MariaDB (XAMPP Compatible)
-- ============================================================================
...
DROP TRIGGER IF EXISTS `hospital_db`.`reduce_medicine_stock_after_prescription`;
DROP PROCEDURE IF EXISTS `hospital_db`.`sp_get_patient_history`;
DROP VIEW IF EXISTS `hospital_db`.`vw_patient_billing_summary`;
DROP VIEW IF EXISTS `hospital_db`.`vw_active_admissions`;
DROP VIEW IF EXISTS `hospital_db`.`vw_doctor_details`;

DROP TABLE IF EXISTS `hospital_db`.`bills`;
DROP TABLE IF EXISTS `hospital_db`.`prescriptions`;
...
-- Default Admin User (Username: admin, Password: password_hash of 'admin123')
...
-- ----------------------------------------------------------------------------
-- VIEW 1: Doctor Details with Department Name & Total Appointments
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- VIEW 2: Active Admissions Summary
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- TRIGGER: Automatically reduce medicine stock after prescription is created
-- ----------------------------------------------------------------------------
DELIMITER //
CREATE TRIGGER `reduce_medicine_stock_after_prescription`
...
-- ----------------------------------------------------------------------------
-- STORED PROCEDURE: Get Patient Complete Appointment & Prescription History
-- ----------------------------------------------------------------------------
DELIMITER //
CREATE PROCEDURE `sp_get_patient_history`(IN `p_patient_id` INT)
BEGIN
    -- 1. Patient Profile
    SELECT id, name, gender, age, phone, address, blood_group, created_at
    FROM patients
